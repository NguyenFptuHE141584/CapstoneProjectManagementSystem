...
$(document).ready(function () {
    AjaxCall('/StudentProfile/CheckProfile', 'POST').done(function (response) {
        if (response == true) {
            Swal.fire({
                icon: 'warning',
                title: '<p class="popupTitle">Please complete your information <br/>( NOTICE: Field with * is required )</p>'
            })
...
$('.btnSave').on('click', function () {
    ...
    var specialty = {};
    specialty.specialtyId = $('#specialtyfilter').val();
...
$(document).ready(function () {
    ...
    $('body').on('blur keyup change', '#professionalfilter, #specialtyfilter ,#phoneContactInfo, #emailContactInfo, #faceBookContactInfo, #nameBasicInfo,  #professionalfilter, #specialtyfilter, #expectRoleBasicInfo, #bioInfo', function () {
        if (!validPhone.test($('#phoneContactInfo').val()) || $('#phoneContactInfo').val().length > 10
            || $('#emailContactInfo').val().length > 200 || $('#faceBookContactInfo').val().length > 200
            || $('#nameBasicInfo').val().length > 50 || $('#professionalfilter').val() == 0
            || $('#specialtyfilter').val() == 0 || $('#expectRoleBasicInfo').val().length > 50
            || $('#bioInfo').val().length > 3000
            || $('#professionalfilter').val == 0 || $('#specialtyfilter').val == 0) {
            $('.btnSave').attr('disabled', 'disabled');
            $('.btnSave').css('cursor', 'not-allowed');
            $('.btnSave').css('opacity', '0.5');
        } else {
