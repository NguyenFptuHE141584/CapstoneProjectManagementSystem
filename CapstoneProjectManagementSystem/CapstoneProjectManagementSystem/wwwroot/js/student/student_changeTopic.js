            ...
            for (var i = 0; i < response.length; i++) {
                if (response[i].status == 0) {
                    $('.requestChange').append('<div class="detailFormChange" id="' + response[i].requestID + '">'
                        + '<p>' + count + '</p >'
                        + '<p class="groupName">' + response[i].finalGroup.groupName + '</p>'
                        + '<p>' + response[i].oldTopicNameEnglish + '</p>'
                        + '<p>' + response[i].newTopicNameEnglish + '</p>'
                        + '<p>' + response[i].emailSuperVisor + '</p>'
...
$(document).ready(function () {
    ...
    $('body').on('click', '.submitFormBtn', function () {
        ...
        changeTopicRequests.emailSupervisor = $('#emailSupervisor').val();
...
$(document).ready(function () {
    ...
    var validMail = /[a-z0-9]+@[a-z]+\.[a-z]{2,3}/;
    $('#emailSupervisor').blur(function () {
        if (!validMail.test($('#emailSupervisor').val())) {
            $('#erroremailSupervisor').html('Input abc@gmail.com or abc@fpt.edu.vn');
        } else if ($('#emailSupervisor').val().replace(/\s/g, "").length <= 0) {
            $('#erroremailSupervisor').html('This field is required');
        } else if ($('#emailSupervisor').val().length > 100) {
            $('#erroremailSupervisor').html('Input less than 100 characters');
        } else {
            $('#erroremailSupervisor').html('');
        }
    })
    ...
    $('body').on('blur keyup', '#topicNameEnglish, #topicVietNameseEnglish, #abbreviation, #emailSupervisor, #reasonChangeTopic', function () {
        if ($('#topicNameEnglish').val().replace(/\s/g, "").length <= 0 || $('#topicNameEnglish').val().length > 100
            ...
            || !validMail.test($('#emailSupervisor').val()) || $('#emailSupervisor').val().replace(/\s/g, "").length <= 0 || ($('#emailSupervisor').val().length > 100)
