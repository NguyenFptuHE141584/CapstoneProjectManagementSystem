...
$(document).ready(function () {
    ...
    if ($('#requestPendingBtn').hasClass('active')) {
        GetListChangeTopicRequests(0, searchText, 'none', 0);
    }
})
...
$('#requestPendingBtn,#requestAcceptedBtn,#requestRejectedBtn,#searchNameRequestInput').on('click change', function () {
    ...
    if ($('#requestRejectedBtn').hasClass('active')) {
        GetListChangeTopicRequests(2, searchText, 'none', 0);
    }
});
...
$('body').on('click', '#previousBtn', function () {
    ...
    if ($('#requestPendingBtn').hasClass('active')) {
        if (!(startNum == 1)) {
            GetListChangeTopicRequests(0, searchText, 'previous', startNum);
        }
    }
    if ($('#requestAcceptedBtn').hasClass('active')) {
        if (!(startNum == 1)) {
    ...
    if ($('#requestRejectedBtn').hasClass('active')) {
        if (!(startNum == 1)) {
            GetListChangeTopicRequests(2, searchText, 'previous', startNum);
        }
    }
});
...
$('body').on('click', '#nextbtn', function () {
    ...
    if ($('#requestPendingBtn').hasClass('active')) {
        if ((startNum + numberOfRecordsPerPage - 1) < countResult) {
            GetListChangeTopicRequests(0, searchText, 'next', startNum);
        }
    }
    if ($('#requestAcceptedBtn').hasClass('active')) {
        if ((startNum + numberOfRecordsPerPage - 1) < countResult) {
    ...
    if ($('#requestRejectedBtn').hasClass('active')) {
        if ((startNum + numberOfRecordsPerPage - 1) < countResult) {
            GetListChangeTopicRequests(2, searchText, 'next', startNum);
        }
    }
});
            ...
            if (status == 0) {
                ...
                for (var i = 0; i < response.changeTopicRequests.length; i++) {
                    $('.showRequestTopic').append('<div class="informRequest">'
                        ...
                        + '<p>' + count + '</p>'
                        + '<p>' + response.changeTopicRequests[i].finalGroup.groupName + '</p>'
                        + '<p>' + response.changeTopicRequests[i].oldTopicNameEnglish + '</p>'
                        + '<p>' + response.changeTopicRequests[i].newTopicNameEnglish + '</p>'
                        + '<p>' + response.changeTopicRequests[i].emailSuperVisor + '</p>'
                        + '</div>'
                ...
                if (response == true) {
                    Swal.fire({
                        ...
                        $(document).ready(function () {
                            var searchText = $('#searchNameRequestInput').val();
                            if ($('#requestPendingBtn').hasClass('active')) {
                                GetListChangeTopicRequests(0, searchText);
                            }
                        })
                        accepBtn.removeAttr('disabled', 'disabled');
                        accepBtn.css('cursor', 'pointer');
